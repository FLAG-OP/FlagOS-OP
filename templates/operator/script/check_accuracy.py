...
IMPLS = {
    "reference": ("reference.py", "my_op_reference"),
    "torch": ("kernel/torch_level.py", "my_op_torch"),
    "triton": ("kernel/triton_level.py", "my_op_triton"),
}
...
def main() -> int:
    ...
    print(f"{'case':12s} {'dtype':10s} {'abs_err':>10s} {'rel_err':>10s} 判定")
    print("-" * 56)
    for f in idx["files"]:
    ...
    print(f"结论: {n_pass} pass / {n_fail} fail · 最差 {worst['case']}"
          f"({worst['dtype']}) abs={worst['abs']:.2e} rel={worst['rel']:.2e}")
