#!/usr/bin/env python3
"""按 goldendata/inputs_spec.yaml 生成黄金输入与参考输出。

用法:
  python3 script/gen_golden.py                    # CPU 生成（最可信参考）
  python3 script/gen_golden.py --device p800-kunlunxin
...
HERE = Path(__file__).resolve().parents[1]          # templates/operator/
        ...
        for shape in case["shapes"]:
            for dt_name in case["dtypes"]:
                ...
                for k in range(spec["seeds_per_case"]):
                    ...
                    inputs = []
                    for i in range(2):              # <TODO: 输入个数>
                        t = dist(*shape, generator=g).to(dt) * case["scale"]
                        for sp in case.get("special", []):
                            SPECIALS[sp](t)
                        inputs.append(t.to(args.device))
                    expected = ref_fn(*inputs).cpu()
                    name = f"{case['name']}_{dt_name}_s{k}.pt"
                    torch.save({"inputs": [t.cpu() for t in inputs],
                                "expected": expected, "shape": shape,
                                "dtype": dt_name, "seed": seed,
                                "case": case["name"]},
                               data_dir / name)
