...
|---|---|---|---|---|
| 算子库层 | kernel 直测 | ✅ PASS | ~8s | 精度 6/6、哨兵(确定性+输入敏感)通过 |
| 框架层 | op 注册/分发 | ✅ PASS | ~7s | 注册为 7 个 impl 之一并被精确钉选 |
| 应用层 | framework 真实推理 | ✅ PASS | ~100s | 真实前向调用 2240 次 |
| 跨层 | 算子库层↔框架层一致性 | ✅ PASS | 7.9s | 4×4 全零误差（同 csrc 模块 gelu_and_mul 锚定） |
