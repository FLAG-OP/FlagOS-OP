#!/usr/bin/env python3
"""性能测试（dropout 单算子）: p=0.5 随机路径，短采样 + 同步。

用法:
  python3 script/bench_perf.py --device mlu --dtype float16 --shape 8192 8192
"""
...
def main() -> int:
    ...
    t = bench(lambda: fn(x, args.p, True), warmup=20, iters=100,
              device=args.device)
    bytes_ = 2 * x.numel() * x.element_size()
    print(f"{args.impl} @ {tuple(args.shape)} {args.dtype} p={args.p}: "
          f"{t:.3f}ms ({bytes_ / t / 1e6:.0f} GB/s)")
    return 0
