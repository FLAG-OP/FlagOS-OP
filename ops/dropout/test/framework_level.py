# 应用层验证: 推理式前向消费 dropout（training=False），跨进程验证注册。
...
def _app_forward(device: str):
    """模拟推理前向: eval 模式下 dropout 透传（transformers/vLLM 常见）。"""
    import torch
    ...
    h = F.dropout(x, 0.1, training=False)   # ← 被测 aten::dropout
    return h @ w
...
def _run_child(register: bool, device: str):
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        env = os.environ.copy()
        env["A1_DROPOUT_COUNT_FILE"] = os.path.join(d, "counts.json")
...
def run(profile):
    ...
    assert calls > 0, "应用层未命中 dropout（aten 注册未被消费）"
