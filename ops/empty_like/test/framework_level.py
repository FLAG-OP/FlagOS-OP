# 应用层验证: 推理式前向消费 empty_like，跨进程验证注册。
...
def _app_forward(device: str):
    """模拟应用: 预分配输出 buffer（empty_like）后写入并 matmul。"""
    import torch
    ...
    buf = torch.empty_like(x)               # ← 被测 aten::empty_like
    buf.copy_(x)
    return buf @ w
...
def _app_child(out_path: str, register: bool, device: str) -> None:
    ...
    if register:
        import register as reg
        calls = reg.CALL_COUNT["empty_like"]
...
def _run_child(register: bool, device: str):
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        env = os.environ.copy()
        env["A1_EMPTY_LIKE_COUNT_FILE"] = os.path.join(d, "counts.json")
...
def run(profile):
    ...
    assert calls > 0, "应用层未命中 empty_like"
    output_match = bool(base_t.shape == plug_t.shape
                        and (base_t - plug_t).abs().max() == 0)
    assert output_match, \
        f"注册前后应用输出不一致: {base_meta['sum']} vs {plug_meta['sum']}"
