#!/usr/bin/env python3
"""性能测试（empty_like）: 分配开销，短采样 + 同步。

注意: 逐次分配会触发分配器池增长（known-issues #10），仅作可用性采样。
"""
...
def main() -> int:
    ...
    x = torch.randn(*args.shape, dtype=dt, device=args.device)
    t = bench(lambda: fn(x), warmup=20, iters=100, device=args.device)
    print(f"{args.impl} empty_like {tuple(args.shape)} {args.dtype}: {t:.4f}ms")
    return 0
