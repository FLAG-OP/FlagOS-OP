#!/usr/bin/env python3
"""detach_ 一键编排: 三层测试一次跑完 + 性能回归用例入口。"""
...
def main() -> None:
    ...
    print(f"[detach_] {profile.summary()}")

    import importlib.util

    def _load(name):
        p = OP_DIR / "test" / f"{name}_level.py"
        spec = importlib.util.spec_from_file_location(f"detach__{name}", p)
    ...
    if not all_ok:
        raise SystemExit("[detach_] 存在失败层级")
    print("\n[detach_] 三层全绿。")
...
def perf_cases(profile):
    ...
    def make(fn):
        def _make(p):
            import torch
            x = torch.randn(1024, dtype=torch.float32, device=p.torch_device)
            return lambda: fn(x)
        return _make

    torch_fn = _load_by_path("kernel/torch_level.py", "detach__torch")
    ref = _load_by_path("reference.py", "detach__reference")
    return [PerfCase("ops.detach_.torch", group="ops", level="kernel",
                     make_fn=make(torch_fn)),
            PerfCase("ops.detach_.reference", group="ops", level="kernel",
                     make_fn=make(ref))]
