...
| | 手写 | KernelGen 官方 MCP | 裸 LLM-Track |
|---|---|---|---|
| 正确性 | 36/36 | 7/9（4D mask 错位; CUDA 方言需人工 patch 编译） | 3/5 |
| 性能 S=2k | 1.437ms | 1.523ms（手写 94%） | 2.935ms |

- 官方管线值 1.93ms（裸→官方）、手写再值 0.09ms + 完整语义 + 平台健壮性
- 两个真断层被四阶段验证逐关拦下: 跨芯片方言编译失败、mask 语义浅
- 详见 [intake_llm/REPORT.md](../intake_llm/REPORT.md)（本地模拟）与
  [REPORT_KG.md](../intake_llm/REPORT_KG.md)（官方 MCP，含调试方法论记录）
