# 性能对照实验: 本实现(triton) vs 原生 torch_npu SDPA vs flag_gems 直调。
# 三方同环境 A/B 对照——"修复与性能都要证据"。
#
# 运行: python3 script/bench_perf.py --device ascend910
#
# 方法论（对齐 FlagOS-OP 约定 + 本机实测教训）:
#   - 短采样 ≤100 次（分配器陷阱，known-issues #10）
#   - wall 时间 + torch.npu.synchronize()（异步早退假象）
#   - 每个 dtype×shape 一组，warmup 充分（Triton 编译一次，缓存后公平）
...
def main() -> int:
    ...
    from kernel.torch_level import sdpa_torch
    ...
    print(f"{'shape':18s} {'ours(ms)':>9s} {'native(ms)':>10s} "
          f"{'gems(ms)':>9s} {'ours/native':>11s}")
    print("-" * 66)
    for tag, B, Hq, Hkv, S, D in SHAPES:
        q, k, v = mk(tag, B, Hq, Hkv, S, D)
        causal = True

        t_ours = bench_fn(lambda: sdpa_triton(q, k, v, None, 0.0,
                                              causal, None, Hq != Hkv))
        ...
        try:
            ...
            t_gems = bench_fn(lambda: scaled_dot_product_attention_forward(
