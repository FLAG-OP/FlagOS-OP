...
def _sdpa_kernel(
    ...
    # PATCH-2c (hand): tl.cdiv(张量) 在 ascend 的 range 上疑似只跑 1 块
    # （假设A实证: 第二 K 块贡献丢失）→ 纯整数算术
    if IS_CAUSAL:
        hi_n = (tl.minimum(seq_len_k, (pid_m + 1) * BLOCK_M) + BLOCK_N - 1) // BLOCK_N
    else:
        hi_n = (seq_len_k + BLOCK_N - 1) // BLOCK_N
    for start_n in range(0, hi_n):

        cur_offs_n = start_n * BLOCK_N + tl.arange(0, BLOCK_N)  # PATCH-2d: range 改计数后偏移必须乘 BLOCK_N
        ...
        qk = tl.dot(q, tl.trans(k))  # PATCH-5: drop ieee on fp16 dot
        qk = qk * scale
        ...
        # PATCH-4: p stays fp32-in-dot via cast below
        acc += tl.dot(p.to(v.dtype), v)  # PATCH-4: same-dtype dot (ascend)
