...
def main() -> None:
    ...
    print(f"[detach] {profile.summary()}")

    import importlib.util

    def _load(name):
        p = OP_DIR / "test" / f"{name}_level.py"
        spec = importlib.util.spec_from_file_location(f"detach_{name}", p)
    ...
    if not all_ok:
        raise SystemExit("[detach] 存在失败层级")
    print("\n[detach] 三层全绿。")
...
def perf_cases(profile):
    ...
    torch_fn = _load_by_path("kernel/torch_level.py", "detach_torch")
    ref = _load_by_path("reference.py", "detach_reference")
    return [PerfCase("ops.detach.torch", group="ops", level="kernel",
                     make_fn=make(torch_fn)),
            PerfCase("ops.detach.reference", group="ops", level="kernel",
                     make_fn=make(ref))]
