...
`aten::detach`：返回与 self 共享底层存储、脱离 autograd 的新张量 (requires_grad=False)。判定口径：共享存储 + requires_grad=False + shape/dtype/stride。
...
| 层级 | 结果 | 明细 |
|---|---|---|
| kernel 层 | ✅ | 4 shape × 3 dtype × {requires_grad True/False} + 非连续 view（25 项） |
| 框架层 | ✅ | aten::detach@PrivateUse1（拦截 1 次） |
| 应用层 | ✅ | 子进程前向正常/一致 |
| 黄金 | ✅ | 36/36 |

## 5. 性能

0.0066ms（该类算子为元数据/同步操作，非带宽/算力受限）。
