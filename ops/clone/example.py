...
def main() -> None:
    ...
    print(f"[clone] {profile.summary()}")

    import importlib.util

    def _load(name):
        p = OP_DIR / "test" / f"{name}_level.py"
        spec = importlib.util.spec_from_file_location(f"clone_{name}", p)
...
def perf_cases(profile):
    ...
    triton = _load_by_path("kernel/triton_level.py", "clone_triton")
    torch_fn = _load_by_path("kernel/torch_level.py", "clone_torch")
    ref = _load_by_path("reference.py", "clone_reference")
    return [
        PerfCase("ops.clone.triton", group="ops", level="kernel",
                 make_fn=make(triton), derived=bw),
        PerfCase("ops.clone.torch", group="ops", level="kernel",
                 make_fn=make(torch_fn)),
        PerfCase("ops.clone.reference", group="ops", level="kernel",
                 make_fn=make(ref)),
    ]
