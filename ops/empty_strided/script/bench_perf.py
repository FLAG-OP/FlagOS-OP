...
def main() -> int:
    ...
    args = ap.parse_args()
    fn = _load(*{
        "reference": ("reference.py", "empty_strided_reference"),
        "torch": ("kernel/torch_level.py", "empty_strided_torch"),
    }[args.impl])
    dt = getattr(torch, args.dtype)
    stride = [args.shape[1], 1]
    t = bench(lambda: fn(tuple(args.shape), stride, dtype=dt,
                         device=args.device),
              warmup=20, iters=100, device=args.device)
    print(f"{args.impl} empty_strided {tuple(args.shape)} {stride} "
          f"{args.dtype}: {t:.4f}ms")
    return 0
