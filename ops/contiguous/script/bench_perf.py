#!/usr/bin/env python3
"""性能测试（contiguous 单算子）: 非连续输入 → 连续输出，短采样 + 同步。
...
def main() -> int:
    ...
    x = torch.randn(*args.shape, dtype=dt, device=args.device).t()  # 非连续
    t = bench(lambda: fn(x), warmup=20, iters=100, device=args.device)
    bytes_ = 2 * x.numel() * x.element_size()
    print(f"{args.impl} @ {tuple(args.shape)} {args.dtype} (transposed): "
