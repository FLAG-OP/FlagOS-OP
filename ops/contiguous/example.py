#!/usr/bin/env python3
"""contiguous 一键编排: 三层测试一次跑完 + 性能回归用例入口。"""
...
def main() -> None:
    ...
    print(f"[contiguous] {profile.summary()}")

    import importlib.util

    def _load(name):
        p = OP_DIR / "test" / f"{name}_level.py"
        spec = importlib.util.spec_from_file_location(f"contiguous_{name}", p)
    ...
    if not all_ok:
        raise SystemExit("[contiguous] 存在失败层级")
    print("\n[contiguous] 三层全绿。")
...
def _load_by_path(rel: str, entry: str):
    import importlib.util
    p = OP_DIR / rel
    key = "contiguous_" + rel.replace("/", "_")[:-3]
...
def perf_cases(profile):
    ...
    triton = _load_by_path("kernel/triton_level.py", "contiguous_triton")
    torch_fn = _load_by_path("kernel/torch_level.py", "contiguous_torch")
    ref = _load_by_path("reference.py", "contiguous_reference")
    return [
        PerfCase("ops.contiguous.triton", group="ops", level="kernel",
                 make_fn=make(triton), derived=bw),
        PerfCase("ops.contiguous.torch", group="ops", level="kernel",
                 make_fn=make(torch_fn)),
        PerfCase("ops.contiguous.reference", group="ops", level="kernel",
                 make_fn=make(ref)),
    ]
