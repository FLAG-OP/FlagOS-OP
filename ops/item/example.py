...
def main() -> None:
    ...
    print(f"[item] {profile.summary()}")

    import importlib.util

    def _load(name):
        p = OP_DIR / "test" / f"{name}_level.py"
        spec = importlib.util.spec_from_file_location(f"item_{name}", p)
...
def perf_cases(profile):
    ...
    torch_fn = _load_by_path("kernel/torch_level.py", "item_torch")
    ref = _load_by_path("reference.py", "item_reference")
    return [PerfCase("ops.item.torch", group="ops", level="kernel",
                     make_fn=make(torch_fn)),
            PerfCase("ops.item.reference", group="ops", level="kernel",
                     make_fn=make(ref))]
