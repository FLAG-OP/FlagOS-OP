# 应用层验证: 推理式前向消费 _local_scalar_dense，跨进程验证注册。
...
def _app_child(out_path: str, register: bool, device: str) -> None:
    ...
    if register:
        import register as reg
        calls = reg.CALL_COUNT["_local_scalar_dense"]
...
def _run_child(register: bool, device: str):
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        env = os.environ.copy()
        env["A1_LOCALSCALARDENSE_COUNT_FILE"] = os.path.join(d, "counts.json")
...
def run(profile):
    ...
    assert calls > 0, "应用层未命中 _local_scalar_dense"
