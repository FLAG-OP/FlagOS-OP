#!/usr/bin/env python3
"""性能测试（_local_scalar_dense）: 短采样 + 同步。"""
...
def main() -> int:
    ...
    args = ap.parse_args()
    fn = _load(*{"reference": ("reference.py", "_local_scalar_dense_reference"),
                 "torch": ("kernel/torch_level.py", "_local_scalar_dense_torch")}[args.impl])
    import torch
    x = torch.randn(1, dtype=torch.float32, device=args.device)
    t = bench(lambda: fn(x), warmup=20, iters=100, device=args.device)
    print(f"{args.impl} _local_scalar_dense: {t:.4f}ms")
    return 0
