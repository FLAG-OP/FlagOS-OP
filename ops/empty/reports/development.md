...
`aten::empty`：分配 size 形状的未初始化张量，只保证元数据。**正确性只由元数据定义**。

## 3. 实现说明

[kernel/torch_level.py](../kernel/torch_level.py)：meta 推 stride + empty_strided。
...
8192² fp32 分配：0.0107ms（分配器受限，仅可用性采样）。
...
三层全绿，元数据 48/48 一致；无 Triton/硬件级（分配算子固有）。
