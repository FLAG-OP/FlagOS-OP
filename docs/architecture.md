...
为什么要有这张矩阵？因为一个 kernel 在隔离环境里正确，距离它在
真实推理里正确工作，中间还隔着几道会各自出错的关口: 注册进了框架
却没被策略选中、被选中了却改变了数值语义、数值没变却破坏了端到端
输出。本库开发过程中这几类问题都真实出现过——厂商 kernel 注册后被
默认实现绕过、aten 注册被 flag_gems 同名算子覆盖、自研 kernel 数值
正确但端到端输出混沌分叉。所以每格都要单独验证，且**独立可跑**:
排障时只跑出问题的一格，几十秒定位问题在哪一层。
...
矩阵的读法是"先列后行": 先确定你要验证的位置（列，物理栈的层），
再选接入机制（行，路线）。例如写一个 Triton 的 `torch.bmm` 替换，
验证路径就是 A1 行的三格——kernel 直测确认数值、aten 注册确认拦截
生效、framework 双跑确认端到端不破坏；跨层一致性再横向确认"注册"
这个动作本身没有偷换语义。

<a id="levels"></a>
## 开发级别（kernel 用什么写）

三个级别按工具链抽象度从高到低排列。选哪个级别，本质是在开发
效率和性能天花板之间取舍: torch 级组合现成 ATen 算子就能完成融合
（b-fullstack 一个 C++ 文件即闭环），但不产生新设备码，性能受框架
调度限制；Triton 级写法接近 NumPy 却能产出真正的设备码，BMM 样例
在这条路上跑赢了原生 1.25 倍；硬件级天花板最高，但依赖厂商工具链——
本机无昆仑芯 SDK，只能直测厂商预编译 kernel，而实测 5 个里 3 个有
缺陷（见 known-issues）。
