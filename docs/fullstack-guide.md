...
> **起点**: `cp -r templates/operator <你的算子名>/`——样板已含参考实现、
...
第 0 步定好了判卷标准，这一步才动手写实现。最常见的时间浪费是
跳过参考直接写 kernel，结果数值对不上时无法区分"kernel 错了"还是
"语义理解错了"——两个都改一遍的事在本库开发中真实发生过。
...
这一步在注册之前做，是为了把变量隔离开: 直测不过，问题一定在
kernel；直测过了再注册，注册之后出的问题才指向注册环节。如果
两步混在一起跑，排障时每次都要猜问题在哪一侧。
...
前三步都在可控环境里，这一步进入真实的 vLLM 进程模型——最大的
心智转变是接受"注册代码不在你眼前执行": v1 前向跑在 EngineCore
子进程里，你在主进程做的任何注册它都看不见，注入必须通过环境变量
和插件发现机制传递（A1 还要额外过 sitecustomize 桥）。
